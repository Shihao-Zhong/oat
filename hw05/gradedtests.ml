...
let student_big_tests = [
  ("hw5programs/sp18_hw5/huffman.oat", "", "c=000 b=001 d=01 a=1 0")
  ; ("hw5programs/sp18_hw5/linked_list.oat", "", "1")
  ; ("hw5programs/sp18_hw5/pseudorandom_quicksort_deque.oat", "", "164")
  ; ("hw5programs/sp18_hw5/bst.oat", "", "2 3 4 5 6 10 11 25 | 3 4 2 6 10 11 5 25 0")
  ; ("hw5programs/sp18_hw5/neuron.oat", "", "11")
  ; ("hw5programs/sp18_hw5/student.oat", "", "90")
  (* Bug in ExtractMin, not compiler-related *)
  ; ("hw5programs/sp18_hw5/heap.oat", "", "2")
  ; ("hw5programs/sp18_hw5/fun_with_lists.oat", "", "10")
  ; ("hw5programs/sp18_hw5/hashmap.oat", "", "3") 
  ; ("hw5programs/sp18_hw5/functional.oat", "", "15 120 false 2 2 2 0")
  ; (* Not good solution, trivially returns 1 *)
  ("hw5programs/sp18_hw5/dfs.oat", "", "1") 
  (* Submission by atter wrong, does not even typecheck: 
     ; ("hw5programs/sp18_hw5/lca.oat", "", "20" ) 
  *)
  ; ("hw5programs/sp18_hw5/battleitout.oat", "", "1")
]

let petrosky_struct_tests = [
  ("ok sub_subrstruct",
   (fun () ->
     let structA = ("A", [{ fieldName = "x"; ftyp = TInt };
                          { fieldName = "y"; ftyp = TInt }]) in
    let structB = ("B", [{ fieldName = "x"; ftyp = TInt }]) in
    let tc = { locals = []; globals = []; structs = [structA; structB] } in
    if Typechecker.subtype tc (TRef (RStruct "A")) (TRef (RStruct "B")) then ()
    else failwith "should not fail"));
  ("err sub_subrstruct", (fun () ->
    let structA = ("A", [{ fieldName = "x"; ftyp = TInt };
                         { fieldName = "y"; ftyp = TInt }]) in
    let structB = ("B", [{ fieldName = "x"; ftyp = TInt }]) in
    let tc = { locals = []; globals = []; structs = [structA; structB] } in
    if Typechecker.subtype tc (TRef (RStruct "B")) (TRef (RStruct "A")) then
      failwith "should not succeed" else ()));
]

let scaby_function_tests = [
  "subtype_subrfunct",
  (fun () ->
    let f1 = TRef(RFun([TRef(RArray(TBool))], RetVoid)) in
    let f2 = TRef(RFun([TNullRef(RArray(TBool))], RetVoid)) in
    if Typechecker.subtype Tctxt.empty f2 f1 then ()
    else failwith "should not fail")                                                                                     
  ; ("no_subtype_subrfunct",
     (fun () ->
       let f1 = TRef(RFun([TRef(RArray(TBool))], RetVoid)) in
       let f2 = TRef(RFun([TNullRef(RArray(TBool))], RetVoid)) in
       if Typechecker.subtype Tctxt.empty f1 f2 then
         failwith "should not succeed" else ())
  )
]
  
let aen_subarray_tests = [
  ("ok sub_subrarray", (fun () ->
    let structA = ("A", [{ fieldName = "x"; ftyp = TInt };
                         { fieldName = "y"; ftyp = TInt }]) in
    let structB = ("B", [{ fieldName = "x"; ftyp = TInt }]) in
    let tc = { locals = []; globals = []; structs = [structA; structB] } in
    if Typechecker.subtype tc
      (TRef (RArray (TRef (RStruct "A"))))
      (TRef (RArray (TRef (RStruct "A")))) then ()
    else failwith "should not fail"));
  ("err sub_subrarray", (fun () ->
    let structA = ("A", [{ fieldName = "x"; ftyp = TInt };
                         { fieldName = "y"; ftyp = TInt }]) in
    let structB = ("B", [{ fieldName = "x"; ftyp = TInt }]) in
    let tc = { locals = []; globals = []; structs = [structA; structB] } in
    if Typechecker.subtype tc
      (TRef (RArray (TRef (RStruct "A"))))
      (TRef (RArray (TRef (RStruct "B")))) then
      failwith "should not succeed" else ()));
]

  
let sakhavan_binop_tests = [
  "typing_binop_pass",
  (fun () ->
    let e = no_loc @@ Bop(And, no_loc @@ CBool true, no_loc @@ CBool true) in
    try
      let _ = Typechecker.typecheck_exp Tctxt.empty e in
      ()
    with 
  ...
  ; ("typing_binop_fail",
     (fun () ->
       let e = no_loc @@ Bop(And, no_loc @@ CInt 1L, no_loc @@ CInt 1L) in
       try
      	 let _ = Typechecker.typecheck_exp Tctxt.empty e in
...
let lomay_carr_ty_unit_tests = [
  ("ok carr_of_sub",
   (fun () ->
     let s1 = ("S1", [{ fieldName = "b"; ftyp = TBool };
                      { fieldName = "val"; ftyp = TInt }]) in
     let s2 = ("S2", [{ fieldName = "b"; ftyp = TBool }]) in 
     let ins_of_s1 = no_loc @@ CStruct ("S1", [("b", no_loc(CBool false));
                                               ("val", no_loc(CInt 10L))]) in 
     let ins_of_s2 = no_loc @@ CStruct ("S2", [("b", no_loc(CBool true))]) in 
     let tc = { locals = []; globals = []; structs = [s1;s2] } in 
     let s2_ty = Typechecker.typecheck_exp tc ins_of_s2 in
     let arr = no_loc @@ CArr (s2_ty, [ins_of_s1;ins_of_s2])in
     let f =  (fun () -> let _ = Typechecker.typecheck_exp tc arr in ()) in
     typecheck_correct f ()
   ))
  ; ("err carr_of_super",
     (fun () ->
       ...
       let ins_of_s2 = no_loc @@ CStruct ("S2", [("b", no_loc(CBool true))]) in 
       let tc = { locals = []; globals = []; structs = [s1;s2] } in 
       let s1_ty = Typechecker.typecheck_exp tc ins_of_s1 in
       let arr = no_loc @@ CArr (s1_ty, [ins_of_s1;ins_of_s2])in
       let f =  (fun () -> let _ = Typechecker.typecheck_exp tc arr in ()) in
       typecheck_error f ()
     ))
]

let javarun_array_null_tests = [
  "subtype_arrQ_arrQ",
  (fun () ->
    if Typechecker.subtype Tctxt.empty
      (TNullRef (RArray TInt)) (TNullRef (RArray TInt) ) then ()
    else failwith "should not fail")
  ; ("no_subtype_arrQ_arrQ",
     (fun () ->
       if Typechecker.subtype Tctxt.empty
         (TNullRef (RArray TInt)) (TRef (RArray TInt)) then
         failwith "should not succeed" else ())
  )
]

let kathdix_func_type_tests = [
  "typecheck_ty_wf_reftokokfunt_good", (
    fun () -> try Typechecker.typecheck_ty (no_loc (CBool true)) Tctxt.empty
                    (TRef (RFun ([ TInt; TBool ], RetVoid)))
      with Typechecker.TypeError _ -> failwith "should not fail");
  "typecheck_ty_wf_reftokokfunt_bad_rty", (
    fun () -> try Typechecker.typecheck_ty (no_loc (CBool true)) Tctxt.empty
                    (TRef (RFun ([], RetVal (TRef (RStruct "S1")))));
                  failwith "should fail" with Typechecker.TypeError _ -> ());
]

let mattch_length_type_tests =
  let exp_good  = no_loc (Length (no_loc (CArr (TInt, [])))) in
  let a_good () = ignore @@ Typechecker.typecheck_exp Tctxt.empty exp_good in
  let exp_bad   = no_loc (Length (no_loc (CBool true))) in
  let a_bad ()  = ignore @@ Typechecker.typecheck_exp Tctxt.empty exp_bad in
  [ "typecheck_length_good",
    typecheck_correct a_good
  ; "typecheck_length_bad",
    typecheck_error a_bad
  ]

let xiaov_eq_tests = [  
  "Eq_same_types",
  typecheck_correct (fun() ->
    let _ = Typechecker.typecheck_exp Tctxt.empty
      (no_loc (Bop (Eq, no_loc (CInt 5L), no_loc(CInt 6L))))
    in ())
  ; "Eq_ref_nullref",
  typecheck_error (fun() ->
    let _ = Typechecker.typecheck_exp Tctxt.empty
      (no_loc (Bop (Eq, no_loc (CNull RString), no_loc(CStr "str"))))
    in ())
]

(* 
let matriley_distinct_args_tests = [
  ("should pass on distinct arg names", fun () ->
    try
      let valid_fdecl = {
        frtyp = RetVoid;
        fname = "foo";
        args = [(TBool, "a"); (TBool, "b")];
        body = [no_loc @@ Ret None]
      } in
      Typechecker.typecheck_fdecl Tctxt.empty valid_fdecl (no_loc None)
    with
    | Typechecker.TypeError s -> failwith "should pass"
  );
  ("should fail on duplicate arg names", fun () ->
    try 
      let invalid_fdecl = {
        frtyp = RetVoid;
        fname = "gotcha";
        args = [(TBool, "a"); (TBool, "a")];
        body = [no_loc @@ Ret None]
      } in
      let _ = Typechecker.typecheck_fdecl Tctxt.empty invalid_fdecl
        (no_loc None) in
      failwith "should not pass"
    with 
...
let wglisson_cast_id_tests = [
  "if? id pass",
  (fun () ->
    let match_id = "x" in
    let st = ("ST", [{ fieldName = "a"; ftyp = TBool }]) in
    let ret_of_int i = no_loc
      @@ Ret (Some (no_loc @@ CInt (Int64.of_int i))) in
    let struct_exp =  no_loc
       @@ CStruct ("ST", [("a", no_loc @@ CBool true)]) in 
    (* let var_exp = CNull (RStruct "ST") in *)
    let ifstmt = no_loc
      @@ Cast (RStruct "ST", match_id, struct_exp,
               [ret_of_int 10], [ret_of_int 0] ) in
    let tc = { locals = []; globals = []; structs = [st] } in
    let f = (fun f ->
      let _ = Typechecker.typecheck_stmt tc ifstmt (RetVal TInt) in ()) in
    typecheck_correct f ())
  ...
  ; ("if? id fail",
     (fun () ->
       ...
       let var_exp = CNull (RStruct "ST") in
       let ifstmt = no_loc
         @@ Cast (RStruct "ST", match_id, no_loc @@ var_exp,
                  [ret_of_int 10], [ret_of_int 0] ) in
       let tc = { locals = [loc_id]; globals = []; structs = [st] } in
       let f = (fun f ->
         let _ = Typechecker.typecheck_stmt tc ifstmt (RetVal TInt) in ()) in
       typecheck_error f ())
  )
...
let atter_tests = [
  "func1",
  (fun () ->
    let f1 = TRef (RFun ([TRef (RStruct "a"); TRef (RArray TBool)], RetVoid)) in
    let f2 = TRef (RFun ([TRef (RStruct "a"); TRef(RArray(TBool))], RetVoid)) in
    if Typechecker.subtype Tctxt.empty f2 f1 then ()
    else failwith "You should have passed this one")                                                                                     
  ; ("func2",
     (fun () ->
       let f1 = TRef(RFun([TBool; TNullRef(RArray(TInt))], RetVoid)) in
       let f2 = TRef(RFun([TNullRef (RArray(TBool)); TBool], RetVoid)) in
       if Typechecker.subtype Tctxt.empty f1 f2 then
         failwith "should not succeed" else ())
  )
]
...
let smumick_tests = [
  "using id in local is fine",
  typecheck_correct (
    fun() ->
      let ctxt = Tctxt.add_local Tctxt.empty "test" (TInt) in
      let _ = Typechecker.typecheck_exp ctxt
        (no_loc (Bop (Eq, no_loc (CInt 5L), no_loc(Id "test"))))
      in ()
  )

  ; 
  "using id that's not in local is not ok",
  typecheck_error (
    fun() ->
      let _ = Typechecker.typecheck_exp Tctxt.empty
        (no_loc (Bop (Eq, no_loc (CInt 5L), no_loc(Id "test"))))
  ...
  ;
  "comparing two different types for equality invalid",
  typecheck_error (
    fun() ->
      let _ = Typechecker.typecheck_exp Tctxt.empty
        (no_loc (Bop (Eq, no_loc (CArr (TBool, [])), no_loc(CBool true))))
      in ()
  )
]
    
let student_small_tests =
  petrosky_struct_tests
  @ scaby_function_tests
  @ aen_subarray_tests
  @ sakhavan_binop_tests
  @ lomay_carr_ty_unit_tests
  @ javarun_array_null_tests
  @ kathdix_func_type_tests
  @ mattch_length_type_tests
  (* @ matriley_distinct_args_tests *)
  (* @ wglisson_cast_id_tests *)
  @ atter_tests
  @ smumick_tests
    
let manual_tests : suite = [
    GradedTest ("Posted Piazza Test Case", 10,
              [("manually", assert_eq true false)]
    )
  ; GradedTest ("Small tests", 1, student_small_tests)
  ; GradedTest ("Big tests", 4, executed_oat_file student_big_tests)
  ]
