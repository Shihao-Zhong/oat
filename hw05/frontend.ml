...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.NewArr (elt_ty, e1, id, e2) ->    
    ...
    let arr_ty, arr_op, alloc_code = oat_alloc_array tc elt_ty size_op in
    let arr_id, arr_len_id, ans_id =
      gensym "newArr_arr_ptr", gensym "newArr_len_ptr", gensym "newArr_ans" in
    let c = Ctxt.add c arr_id (Ptr arr_ty, Id arr_id) in
    let c = Ctxt.add c arr_len_id (Ptr I64, Id arr_len_id) in
    let arr_id_code = lift
      [ arr_id, Alloca arr_ty
      ; "", Store (arr_ty, arr_op, Id arr_id)] in
    let arr_len_code = lift
      [ arr_len_id, Alloca I64
      ; "", Store (I64, size_op, Id arr_len_id)] in
    let vdecl, guard, after, body =
      (id, no_loc @@ CInt 0L),
      no_loc @@ Bop(Lt, no_loc @@ Id id, no_loc @@ Id arr_len_id),
      no_loc @@ Assn(
        no_loc @@ Id id,
        no_loc @@ Bop(Add, no_loc @@ Id id, no_loc @@ CInt 1L)),
      no_loc @@ Assn(
        no_loc @@ Index(no_loc @@ Id arr_id, no_loc @@ Id id), e2) in
    let for_stmt = no_loc @@ For([vdecl], Some guard, Some after, [body]) in
    let _, new_arr_code = cmp_stmt tc c Void for_stmt in
    arr_ty, Id ans_id,
      size_code >@ alloc_code >@ arr_id_code >@ arr_len_code >@ new_arr_code >@ lift
      [ans_id, Load(Ptr arr_ty, Id arr_id)]
